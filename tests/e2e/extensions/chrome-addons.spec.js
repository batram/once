...
test("a scripted add-on runs in Chrome's sandbox page", async () => {
  ...
  try {
    ...
    const tray = alpha.getByTestId("addon-tray")
    await tray.getByRole("textbox").fill("Who uses it?")
    ...
    await otherPanel.goto(`chrome-extension://${extensionId}/static/sidepanel.html?once-e2e=1`)
    await expectExtensionReady(otherPanel, evidence)
    await page.bringToFront()
    const openedConversation = context.waitForEvent("page")
    await tray.getByTestId("addon-tray-continue").click()
    const conversation = await openedConversation
    await expect(conversation.locator(".addon_conversation_messages")).toContainText("Developers use it")
    await expect(page.locator("#selected_container story-item")).toHaveAttribute("data-href", source.urls.alpha)
    await otherPanel.close()
    await tray.getByRole("button", { name: "Close", exact: true }).click()
    await expect(conversation.locator(".addon_conversation_messages")).toContainText("Developers use it")
    await expect(conversation.locator(".addon_conversation_notice")).toBeHidden()
    await page.getByTestId("settings-menu").click()
    await page.locator('[data-settings-target="addons"]').click()
    ...
    await page.close()
    await expect(conversation.locator(".addon_conversation_notice")).toContainText("closed")
