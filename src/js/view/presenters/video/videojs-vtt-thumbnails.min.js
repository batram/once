 ...
 */
!function(t, e) {
    "object" == typeof exports && "undefined" != typeof module ? module.exports = e(require("video.js")) : "function" == typeof define && define.amd ? define(["video.js"], e) : t.videojsVttThumbnails = e(t.videojs)
}(this, function(i) {
    "use strict";
        ...
        function t(i) {
            ...
            function a(t, e) {
                try {
                    var r = i[t](e)
                      , s = r.value;
                    s instanceof l ? Promise.resolve(s.value).then(function(t) {
                        a("next", t)
                    }, function(t) {
                        a("throw", t)
    ...
    var o = {}
      ...
      , a = function() {
        ...
        return r.prototype.src = function(t) {
            this.resetPlugin(),
            this.options.src = t,
            this.initializeThumbnails()
        }
        ...
        r.prototype.setupThumbnailElement = function(t) {
            ...
            r.setAttribute("class", "vjs-vtt-thumbnail-display"),
            this.progressBar.appendChild(r),
            this.thumbnailHolder = r,
        ...
        r.prototype.onBarMousemove = function(t) {
            this.updateThumbnailStyle(this.getXCoord(this.progressBar, t.clientX), this.progressBar.offsetWidth)
        }
        ,
        ...
        let display_value = document.querySelector('.vjs-time-tooltip').innerText.split(':');
            let time = parseInt(display_value[0]);

            if(display_value.length == 2){
              time = (parseInt(display_value[0]) * 60) + parseInt(display_value[1]);
            } else if(display_value.length == 3){
              time =  (parseInt(display_value[0]) * 3600) + (parseInt(display_value[1]) * 60) + parseInt(display_value[2]);
            }
            
            var s = this.getStyleForTime(time);
            if (!s)
                return;// this.hideThumbnailHolder();
            var i = (1 - (e - t) / e) * e;
            
            this.thumbnailHolder.style.transform = "translateX(" + i + "px) scale(1.2)";
            this.thumbnailHolder.style.marginLeft = "-" + parseInt(this.thumbnailHolder.style.width) / 2 + "px";
            this.thumbnailHolder.style.top = "-" + (parseInt(this.thumbnailHolder.style.height) + 20) + "px";
            
            if(!this.lastStyle || this.lastStyle !== s)
        ...
        r.prototype.processVtt = function(t) {
            ...
            return t.split(/[\r\n][\r\n]/i).forEach(function(t) {
                if (t.match(/([0-9]{2}:)?([0-9]{2}:)?[0-9]{2}(.[0-9]{3})?( ?--> ?)([0-9]{2}:)?([0-9]{2}:)?[0-9]{2}(.[0-9]{3})?[\r\n]{1}.*/gi)) {
                    var e = t.split(/[\r\n]/i)
                      , r = e[0].split(/ ?--> ?/i)
                      , s = r[0]
                      , i = r[1]
        ...
        r.prototype.getFullyQualifiedUrl = function(t, e) {
            ...
            return 0 <= t.indexOf("//") ? t : 0 === e.indexOf("//") ? [e.replace(/\/$/gi, ""), this.trim(t, "/")].join("/") : 0 < e.indexOf("//") ? [this.trim(e, "/"), this.trim(t, "/")].join("/") : t
        }
        ,
        ...
        r.prototype.getVttCss = function(t) {
            ...
            if(!window.vtt_fetched){
              window.vtt_fetched = []
            }
            
            if(!window.vtt_fetched.includes(s.image)){
              window.vtt_fetched.push(s.image);
             // fetch(s.image)
            }
            
            e.background = 'url("' + s.image + '") no-repeat -' + s.x + "px -" + s.y + "px",
            e.width = s.w + "px",
            e.height = s.h + "px",
            e.url = s.image;
            
            let holder = document.createElement('div');
            holder.style.background = e.background;
        ...
        r.prototype.getSecondsFromTimestamp = function(t) {
            ...
            return parseInt(3600 * e.hours + 60 * e.minutes + e.seconds + e.milliseconds / 1e3)
        }
        ,
