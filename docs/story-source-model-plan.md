...
The source model began as a prerequisite for per-source cache timing, now tracked separately in
[story-cache-timing-plan.md](story-cache-timing-plan.md). A source had no identity: it was one opaque
line of text, and that text already keyed `AppRuntime.sourceErrors`, `processingSources`,
`SettingsPanel.sourceErrors`, the textarea highlighter, and `LoaderInsights.issueId`. Editing the
line broke every association silently.

Giving sources durable identity and typed configuration fixes the underlying model:
...
export interface StorySource {
  ...
  cacheMinutes?: number   // reserved for the follow-on cache-timing plan
...
```json
{
  ...
  "sources": [
    {"id":"src_7c1e0b3a","url":"https://news.ycombinator.com/","groupId":"grp_1f4a9c02"},
...
- `StoryParser.options` gains a required unique `id` (`geny`, `hackernews`, `jsonselect`,
  ...
  `source.collector` and may key future persisted settings — so renaming one requires an alias plus a
...
- `structured/form.ts`: the field descriptor grows a `kind`
  ...
  Source form: URL, Label, Collector, Enabled, Group. Cache timing adds its field separately.
...
- Editing url, label, group, collector or config preserves the source id.
