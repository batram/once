...
// the only bridge that page has: connect under its token, hear the shell's
// snapshots, and send the reader's input back. The preload stays attached if
// the tab later navigates elsewhere, so it exposes nothing outside the page's
// own scheme, and main checks every sender again.
...
if (location.protocol === "once-addon:" && location.host === "conversation") {
  const bridge: ElectronConversationPageBridge = {
    connect: token => ipcRenderer.invoke(ELECTRON_IPC.addonsConversationConnect, token),
    onState(handler) {
    ...
    send: (token, command) => ipcRenderer.send(ELECTRON_IPC.addonsConversationCommand, token, command)
  }
